...
@section('title', 'تعديل/رد على إشارة - الشبكة الوطنية للطوارئ')
@section('page-title', 'تعديل / رد على إشارة')
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div class="bg-primary p-6 text-white flex items-center justify-between">
                <h3 class="text-2xl font-black flex items-center gap-3">
                    <i class="fas fa-reply text-accent"></i> تعديل / رد على إشارة
                </h3>
                <div class="text-left">
                    <span class="text-xs opacity-60 uppercase block">كود الإشارة</span>
                    <span class="text-xl font-mono font-bold">{{ $mainSignal->MainSignalCode }}</span>
                </div>
            </div>

            <form method="POST" action="{{ route('signals.update', $mainSignal) }}" id="signal-form" class="p-8 space-y-6">
                @csrf
                @method('PUT')
...
{{-- نفس القالب بالظبط اللي في شاشة الإضافة --}}
            ...
            <div class="space-y-1">
                ...
                <div class="flex gap-6 py-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" class="signal-type" value="إشارة لاسلكية" style="accent-color:#e67e22" checked>
                        <span class="font-bold">إشارة لاسلكية</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" class="signal-type" value="رصد مرئي" style="accent-color:#e67e22">
...
@push('scripts')
    <script>
        ...
        function addSignalCard(existing = null) {
            ...
            timeInput.value = existing?.time ?? nowTime;
            timeInput.name = `signals[${index}][time]`;
            senderInput.name = `signals[${index}][sender]`;
            if (existing?.sender) senderInput.value = existing.sender;
            contentInput.name = `signals[${index}][content]`;
            if (existing?.content) contentInput.value = existing.content;
            subjectInput.name = `signals[${index}][subject]`;
            if (existing?.subject) subjectInput.value = existing.subject;
            typeInputs.forEach(input => {
                input.name = `signals[${index}][type]`;
                if (existing?.type) input.checked = (input.value === existing.type);
            });
            ...
            wrapper.querySelectorAll('.authority-toggle').forEach(btn => {
                ...
                const initialState = existing?.authorities?.[authorityName] ?? '';
                hiddenInput.value = initialState;
                btn.dataset.state = initialState;
                btn.appendChild(hiddenInput);
                applyAuthorityState(btn, initialState);
        ...
        // تحميل كل الكروت القديمة بتاعة الثريد ده أول ما الصفحة تفتح
        const existingUnits = @json($existingUnits);
        existingUnits.forEach(unit => addSignalCard(unit));
    </script>
@endpush