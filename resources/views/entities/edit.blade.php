...
@section('title', 'تعديل جهة - نظام التمامات')
@section('page-title', 'تعديل بيانات الجهة')
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div class="bg-primary p-6 text-white">
                <h3 class="text-2xl font-black flex items-center gap-3">
                    <i class="fas fa-building text-accent"></i> تعديل بيانات الجهة
                </h3>
            </div>

            <form method="POST" action="{{ route('entities.update', $entity) }}" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="main_location" value="{{ old('main_location', $entity->main_location) }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none">
                </div>
