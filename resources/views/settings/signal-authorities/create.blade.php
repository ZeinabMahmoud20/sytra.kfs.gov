...
@section('title', 'إضافة جهة إشارة - الإعدادات')
@section('page-title', 'إضافة جهة إشارة جديدة')
...
@section('content')
    <div class="max-w-2xl mx-auto w-full">
        <a href="{{ route('settings.signal-authorities.index') }}"
            ...
            <i class="fas fa-arrow-right"></i> رجوع لجهات الإشارة
        </a>

        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div class="bg-primary p-6 text-white">
                <h3 class="text-2xl font-black flex items-center gap-3">
                    <i class="fas fa-broadcast-tower text-accent"></i> إضافة جهة إشارة جديدة
                </h3>
            </div>

            <form method="POST" action="{{ route('settings.signal-authorities.store') }}" class="p-8 space-y-6">
                @csrf
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="SIGNAL_NAME" value="{{ old('SIGNAL_NAME') }}" required
                        ...
                        placeholder="مثال: هيئة الدفاع المدني">
                </div>
