...
@section('title', 'تعديل جهة إشارة - الإعدادات')
@section('page-title', 'تعديل جهة الإشارة')
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div class="bg-primary p-6 text-white">
                <h3 class="text-2xl font-black flex items-center gap-3">
                    <i class="fas fa-broadcast-tower text-accent"></i> تعديل جهة الإشارة
                </h3>
            </div>

            <form method="POST" action="{{ route('settings.signal-authorities.update', $authority) }}" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="SIGNAL_NAME" value="{{ old('SIGNAL_NAME', $authority->SIGNAL_NAME) }}" required
