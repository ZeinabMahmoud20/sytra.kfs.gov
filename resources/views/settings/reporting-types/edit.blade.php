...
@section('title', 'تعديل نوع بلاغ - الإعدادات')
@section('page-title', 'تعديل نوع البلاغ')
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div class="bg-primary p-6 text-white">
                <h3 class="text-2xl font-black flex items-center gap-3">
                    <i class="fas fa-file-circle-plus text-accent"></i> تعديل نوع البلاغ
                </h3>
            </div>

            <form method="POST" action="{{ route('settings.reporting-types.update', $reportingType) }}" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="REPORT_SORT" value="{{ old('REPORT_SORT', $reportingType->REPORT_SORT) }}" required
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="AUTHORITY" value="{{ old('AUTHORITY', $reportingType->AUTHORITY) }}" required list="authorities-list"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" name="IS_INTERNET" value="1" id="IS_INTERNET"
                        {{ old('IS_INTERNET', $reportingType->IS_INTERNET) ? 'checked' : '' }}
