...
@section('title', 'تعديل جهة إخطار - الإعدادات')
@section('page-title', 'تعديل جهة الإخطار')
...
@section('content')
    <div class="max-w-2xl mx-auto w-full">
        <a href="{{ route('settings.notified-auths.index') }}"
            ...
            <i class="fas fa-arrow-right"></i> رجوع لجهات الإخطار
        </a>

        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div class="bg-primary p-6 text-white">
                <h3 class="text-2xl font-black flex items-center gap-3">
                    <i class="fas fa-bell text-accent"></i> تعديل جهة الإخطار
                </h3>
            </div>

            <form method="POST" action="{{ route('settings.notified-auths.update', $notifiedAuth) }}" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="Notified_Auth" value="{{ old('Notified_Auth', $notifiedAuth->Notified_Auth) }}" required
