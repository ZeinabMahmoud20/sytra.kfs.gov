...
@section('title', 'تعديل جهة مختصة - متابعة التكليفات')
@section('page-title', 'تعديل الجهة المختصة')
...
@section('content')
    <div class="max-w-2xl mx-auto w-full">
        <a href="{{ route('task-entities.index') }}"
            ...
            <i class="fas fa-arrow-right"></i> رجوع لقائمة الجهات المختصة
        </a>

        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div class="bg-primary p-6 text-white">
                <h3 class="text-2xl font-black flex items-center gap-3">
                    <i class="fas fa-building text-accent"></i> تعديل الجهة المختصة
                </h3>
            </div>

            <form method="POST" action="{{ route('task-entities.update', $entity) }}" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-1">
                    <label class="block text-xs font-black text-slate-600">النوع <span class="text-red-500">*</span></label>
                    <select name="type" required
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none">
                        @foreach (['مركز', 'مدينة', 'إدارة', 'مديرية', 'أخرى'] as $type)
                            <option value="{{ $type }}" @selected(old('type', $entity->type) === $type)>{{ $type }}</option>
                        @endforeach
                    </select>
                </div>
